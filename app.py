...
## CHALICE ENTRY POINTS ##
app = Chalice(app_name="data-grinder")
...
def process():
	...
	image_info = process_image(request.query_params["url"], request.query_params["services"])
	return image_info
...
def process_image(URL, services):
	...
	if status == "ok": 
		...
		size = imagesize.get(image_local_path) # (width,height) tuple
		image["width"] = size[0]
		image["height"] = size[1]
